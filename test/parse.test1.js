...
test('parsing vector tiles', function (t) {
    ...
    var layersName = Object.keys(tile.layers);
    for (var index in layersName) {
        var layer = layersName[index];
        console.log('layers in pdf index  -> ', index, ' layer name ->', layer, ' layer length -> ', layer.length);

        for (var i = 0; i < tile.layers[layer].length; i++) {
            //VectorTileFeature
            var feature = tile.layers[layer].feature(i);
            console.log('feature ', i, ' properties is ', feature.properties, ' type is ', feature.type, ' extent is ', feature.extent ,' id is ', feature.id);
        }
    }
    t.test('should extract the tags of a feature', function (t) {
        ...
        // Check line geometry
        t.deepEqual(tile.layers.road.feature(656).loadGeometry(), [[{x: 1988, y: 306}, {x: 1808, y: 321}, {
            x: 1506,
            y: 347
        }]]);
        t.end();
    ...
    t.test('changing first point of a polygon should not change last point', function (t) {
        ...
        t.deepEqual(building, [[{x: 2039, y: -32}, {x: 2035, y: -31}, {x: 2032, y: -31}, {x: 2032, y: -32}, {
            x: 2039,
            y: -32
        }]]);
        building[0][0].x = 1;
